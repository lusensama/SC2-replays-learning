...
from keras.layers import Dense, Dropout, Flatten, Reshape, Concatenate, Input, TimeDistributed, LSTM, GRU
...
def build_lstm_model(infosize, msize, ssize):
    ...
    # interpretation model
    # hidden1 = Dense(256, activation='relu')(merge2)
    value = Dense(1, activation='sigmoid')(lstm)
    output = Reshape((1,))(value)
    model = Model(inputs=[m_input, s_input, i_input], outputs=output)
...
if __name__ == '__main__':
    ...
    eval_test = 'replay_data/'
    if os.path.isfile('primitive_train2.h5'):
    #
    ...
    # model = load_model('primitive_train.h5')
    training_generator = Mygenerator(batch_size, eval_test)
    ...
    model.save('primitive_train2.h5')